function [unwph,iter,erglist] = puma_horse(psi,p,varargin)
...
% figure
fig = figure;plot(0);figaxes = get(fig,'CurrentAxes');
...
%     figure
    while possible_improvment
        ...
        base_start = zeros(size(base,1),size(base,2),cliquesm);
        base_end = zeros(size(base,1),size(base,2),cliquesm);
        t_dkappa = zeros(size(base_kappa,1),size(base_kappa,2),cliquesm);
        a = zeros(size(base,1),size(base,2),cliquesm);
        A = zeros(size(base,1),size(base,2),cliquesm);
        B = zeros(size(base,1),size(base,2),cliquesm);
        C = zeros(size(base,1),size(base,2),cliquesm);
%         D = zeros(size(base,1),size(base,2),cliquesm);
        source = zeros(size(base,1),size(base,2),cliquesm);
        sink = zeros(size(base,1),size(base,2),cliquesm);
...
%             t_dkappa(:,:,t) = (base_kappa-auxili);
            t_dkappa(:,:,t) = (base_kappa-circshift(base_kappa,[cliques(t,1),cliques(t,2)]));
%             auxili2 = circshift(psi_base,[cliques(t,1),cliques(t,2)]);
%             dpsi = auxili2 - psi_base;
            dpsi = circshift(psi_base,[cliques(t,1),cliques(t,2)]) - psi_base;
...
%             sink(:,:,t) = sink(:,:,t) + ((C(:,:,t)-D(:,:,t)).*((C(:,:,t)-D(:,:,t))>0)).*base_end(:,:,t);
            source(:,:,t) = source(:,:,t) + ((A(:,:,t)-C(:,:,t)).*((A(:,:,t)-C(:,:,t))>0)).*base_end(:,:,t);
            sink(:,:,t) = sink(:,:,t) + ((C(:,:,t)-A(:,:,t)).*((C(:,:,t)-A(:,:,t))>0)).*base_end(:,:,t);
        end

%         clear psi_base base_kappa
...
%         clear source sink auxiliar1 base_start base_end sourcefinal sinkfinal
        
        % KAPPA RELABELING
%         [flow,cutside] = mincut(sourcesink,remain);clear sourcesink remain
        [horse,cutside] = mincut(sourcesink,remain);
%         clear sourcesink remain
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%         % Nested the function mincut to solve out of memory problem
...
%         if isempty(fig) == 1
%             fig = figure;plot(0);figaxes = get(fig,'CurrentAxes');
%             set(fig,'CloseRequestFcn',{@my_closereq,fig})
%         end

        if exist('figaxes','var') == 0
            fig = figure;plot(0);figaxes = get(fig,'CurrentAxes');
            set(fig,'CloseRequestFcn',{@my_closereq})
        end
        imagesc(2*pi*kappa + psi,'Parent',figaxes);
        axis image;
        colorbar;
        drawnow;
% %         iter;
% %         erg_actual;
% %         jump_size;
%         clear base_start base_end source sink auxiliar1 auxiliar2 A B C;
    end % while
    title('Puma solution');
    disp(['Finished unwrapping, ' num2str(toc(tstart)/60) ' minutes elapsed'])
    clearvars -EXCEPT unwph iter erglist
    clear global fig figaxes done
end
